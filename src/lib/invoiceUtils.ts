...
export const generateCotisationInvoicePDF = async (ticket: any, event: any) => {
    ...
    try {
        ...
        // Get table bottom Y safely
        const lastTable = (doc as any).lastAutoTable;
        const finalY = lastTable?.finalY || lastTable?.previous?.finalY || (currentY + 25);
        ...
        console.error("Error building invoice PDF:", err);
        return null;
    }
};
...
export const downloadCotisationInvoice = async (ticket: any, event: any) => {
    try {
        ...
        if (doc) {
            ...
            alert("Impossible de générer le fichier PDF de la facture.");
        }
    } catch (err) {
        console.error("Error downloading Cotisation invoice:", err);
