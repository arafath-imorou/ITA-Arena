...
export default function FormSuccessPage() {
        ...
        const fetchSubmission = async () => {
            try {
                const { data, error } = await supabase
                    ...
                    .select("qr_code, form_id, forms(form_type, show_public_results)")
                    .eq("id", subId)
                    .single();
    ...
    const isPoll = formInfo?.form_type === 'poll';
    ...
    return (
        <div style={{ backgroundColor: "#f3f4f6", minHeight: "100vh", padding: "4rem 1rem", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <div style={{ maxWidth: "500px", width: "100%", backgroundColor: "#fff", borderRadius: "12px", overflow: "hidden", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.1)", textAlign: "center", padding: "3rem" }}>
                ...
                <h1 style={{ color: "#0A2E73", fontSize: "1.8rem", marginBottom: "1rem" }}>
                    {isPoll ? "Sondage enregistré !" : "Inscription Confirmée !"}
                </h1>
                <p style={{ color: "#666", marginBottom: "2rem", lineHeight: "1.6" }}>
                    {isPoll 
                        ? "Merci pour votre participation à ce sondage. Vos réponses ont bien été enregistrées." 
